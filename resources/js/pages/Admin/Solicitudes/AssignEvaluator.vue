            ...
            <div class="flex items-center justify-between">
                <div>
                     <nav class="flex text-sm text-gray-500 mb-1">
                        <Link :href="route('admin.solicitudes.index')" class="hover:text-[#1B396A] font-medium">Solicitudes</Link>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <div class="flex justify-between items-start mb-4 border-b border-gray-100 pb-4">
                    ...
                    <span 
                        ...
                        :class="{
                            ...
                            'bg-yellow-100 text-yellow-800 border border-yellow-200': solicitud.status === 'pending'
                        }"
                    >
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                    <div>
                        ...
                        <p class="text-base text-gray-900 font-semibold">{{ solicitud.user?.name || solicitud.user?.data?.name || 'N/A' }}</p>
                    </div>
                    <div>
                        <label class="text-sm text-gray-500 font-medium block mb-1">Campus</label>
                        <p class="text-base text-gray-900 font-medium">{{ solicitud.user?.institucion?.nombre || solicitud.user?.data?.institucion?.nombre || 'N/A' }}</p>
                    </div>
                </div>
                <div class="mt-6">
                    <label class="text-sm text-gray-500 font-medium block mb-1">Convocatoria</label>
                    <p class="text-base text-gray-900 font-medium">{{ solicitud.convocatoria?.nombre || solicitud.convocatoria?.data?.nombre || 'No especificada' }}</p>
                </div>
            </div>
                ...
                <form @submit.prevent="submit">
                    <div class="space-y-6">
                        ...
                        <div class="flex items-center justify-between pt-4 gap-4">
                             <Link :href="route('admin.solicitudes.index')" class="w-full sm:w-auto px-6 py-2.5 border border-gray-300 text-gray-700 hover:bg-gray-50 rounded-lg text-center transition font-bold uppercase text-xs tracking-wider">
                                Cancelar
                            </Link>
                            <button 
                                type="submit" 
