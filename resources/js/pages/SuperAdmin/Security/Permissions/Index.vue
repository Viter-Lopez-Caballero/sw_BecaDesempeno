...
const deletePermission = async (id) => {
    ...
    if (confirmed) {
        router.delete(route(`${props.routeName}destroy`, id), {
            onSuccess: () => {
                alertaExito('¡Eliminado!', 'Permiso eliminado correctamente');
            }
        });
    }
};
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <div class="flex items-center gap-2 mt-2 text-sm">
                        ...
                        <span class="text-gray-900 font-semibold">Permisos</span>
                    </div>
                </div>
                <!-- v-if="useCan('permissions.create')" -->
                <Link v-if="useCan('permissions.create')" :href="route(`${routeName}create`)" class="w-full md:w-auto justify-center px-4 py-2.5 bg-[#1B396A] text-white rounded-lg hover:bg-[#0f2347] transition flex items-center gap-2 font-medium cursor-pointer">
                    ...
                    <table class="w-full text-sm text-left">
                        <thead class="bg-[#1B396A] text-white uppercase text-xs font-semibold">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-4 tracking-wider">
                                    <div @click="sortBy('module_key')" class="flex items-center gap-1 cursor-pointer hover:text-gray-200 transition">
                                        Clave
                                        <svg xmlns="http://www.w3.org/2000/svg" height="16px" viewBox="0 -960 960 960" width="16px" fill="currentColor" :class="{ 'opacity-100': sortField === 'module_key', 'opacity-50': sortField !== 'module_key' }">
                                            <path d="M320-440v-287L217-624l-57-56 200-200 200 200-57 56-103-103v287h-80ZM600-80 400-280l57-56 103 103v-287h80v287l103-103 57 56L600-80Z"/>
                                        </svg>
                                    </div>
                                </th>
                            ...
                            <tr v-for="(permission, index) in permissions.data" :key="permission.id" class="hover:bg-gray-50 transition">
                                ...
                                <td v-if="useCan('permissions.edit') || useCan('permissions.delete')" class="px-6 py-4 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <Link v-if="useCan('permissions.edit')" :href="route(`${routeName}edit`, permission.id)" class="p-2 text-[#1B396A] border border-[#1B396A] rounded-full hover:bg-[#1B396A] hover:text-white transition group cursor-pointer" title="Editar">
                                            <svg xmlns="http://www.w3.org/2000/svg" height="20px" viewBox="0 -960 960 960" width="20px" fill="currentColor">
                                                <path d="M200-200h57l391-391-57-57-391 391v57Zm-80 80v-170l528-527q12-11 26.5-17t30.5-6q16 0 31 6t26 18l55 56q12 11 17.5 26t5.5 30q0 16-5.5 30.5T817-647L290-120H120Zm640-584-56-56 56 56Zm-141 85-28-29 57 57-29-28Z"/>
                                            </svg>
                                        </Link>
                                        <button v-if="useCan('permissions.delete')" @click="deletePermission(permission.id)" class="p-2 text-red-600 border border-red-600 rounded-full hover:bg-red-600 hover:text-white transition group cursor-pointer" title="Eliminar">
                                            <svg xmlns="http://www.w3.org/2000/svg" height="20px" viewBox="0 -960 960 960" width="20px" fill="currentColor">
