...
const onSearch = debounce((value) => {
    router.get(route('evaluador.reconocimientos.index'), {
        search: value,
        rows: rows.value
...
const cleanFilters = () => {
    search.value = '';
    rows.value = 10;
    router.get(route('evaluador.reconocimientos.index'), {}, { preserveState: true, replace: true });
};
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            <tr v-if="reconocimientos.data.length === 0">
                                <td colspan="4" class="px-6 py-12 text-center text-gray-500">
                                    <div class="flex flex-col items-center justify-center">
                                        <svg viewBox="0 0 24 24" class="h-12 w-12 text-gray-300 mb-3" style="fill: currentColor"><path :d="mdiStar"/></svg>
                                        <p>No tienes reconocimientos disponibles aún.</p>
                                    </div>
                                </td>
                            </tr>
                            <tr v-for="item in reconocimientos.data" :key="item.id" class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex items-center justify-center">
                                        <a 
                                            :href="route('evaluador.reconocimientos.download', item.id)" 
                                            target="_blank"
