...
import { mdiSecurity, mdiPencil } from "@mdi/js";
import SectionTitle from "@/Components/SectionTitle.vue";

import { defineProps, computed, inject, reactive, ref, toRefs } from 'vue';
...
export default {
    ...
    setup() {
        ...
        // Asegúrate de que form.permissions esté inicializado como un array vacío
        ...
        const getData = () => {
            ...
            axios.get(route('usuarios.show', form.curp.toUpperCase()))
                .then((response) => {
        ...
        const GeneratePassword = () => {
            ...
            const passwordLength = 8; // Cambia esto si deseas una longitud de contraseña diferente
    ...
    <div class="md:flex md:space-x-4 mb-5">
        <div class="md:w-1/2">
            ...
            <jet-input id="name"
                ...
                v-model="form.name" required placeholder="Nombre del usuario" autocomplete="off" readonly />
            <jet-input-error :message="form.errors.name" />
            <span class="text-xs text-gray-900 dark:text-gray-500 mb-5">Busca el CURP para autocompletar el nombre.</span>
        </div>
        <div class="md:w-1/2">
            ...
            <jet-input id="email"
                ...
                v-model="form.email" required placeholder="Correo Electrónico" />
    ...
    <div class="mb-5">
        ...
        <div class="md:flex md:space-x-4">
            <jet-input id="password"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                v-model="form.password" required placeholder="Contraseña" />
            
            <BaseButton class="max-xl:mt-2" @click="GeneratePassword" :icon="mdiSecurity" type="submit" color="info"
                label="Generar" />
        </div>
        <jet-input-error :message="form.errors.password" />
        <span class="text-xs text-gray-900 dark:text-gray-500 mb-5">Puedes generar una contraseña segura y al azár.</span>
    </div>
    ...
    <table class="min-w-full divide-y">
        ...
        <tbody>
            <tr v-for="item in profiles" :key="item.id" class="flex flex-col md:table-row">
                ...
                <td class="px-6 py-4 md:table-cell" data-label="Nombre Rol">
                    {{ item.name }}
                </td>
                <td class="px-6 py-4 md:table-cell" data-label="Descripcion">
                    {{ item.description }}
                </td>
