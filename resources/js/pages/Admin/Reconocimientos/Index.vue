...
const onSearch = debounce((value) => {
    router.get(route('admin.reconocimientos.index'), { 
        search: value, 
        rows: rows.value,
...
const cleanFilters = () => {
    search.value = '';
    rows.value = 10;
    router.get(route('admin.reconocimientos.index'), {}, { preserveState: true, replace: true });
};

const toggleReconocimiento = (userId, convocatoriaId, currentStatus, solicitudesRevisadas) => {
    if (solicitudesRevisadas === 0) {
        alertaError('Error', 'El evaluador no ha revisado solicitudes de esta convocatoria');
        return;
    }

    router.post(route('admin.reconocimientos.toggle'), {
        user_id: userId,
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            <tr v-for="(item, index) in reconocimientos.data" :key="`${item.evaluador_id}-${item.convocatoria_id}`" class="hover:bg-gray-50 transition">
                                <td class="px-6 py-4 font-medium text-gray-900">{{ (reconocimientos.current_page - 1) * reconocimientos.per_page + index + 1 }}</td>
                                <td class="px-6 py-4 font-semibold text-gray-800">{{ item.evaluador_nombre }}</td>
                                <td class="px-6 py-4 text-gray-600">{{ item.convocatoria_nombre }}</td>
                                <td class="px-6 py-4 text-center">
                                    <div class="flex items-center justify-center">
                                        <button
                                            @click="toggleReconocimiento(item.evaluador_id, item.convocatoria_id, item.activo, item.solicitudes_revisadas)"
                                            :disabled="item.solicitudes_revisadas === 0"
                                            :class="[
                                                ...
                                                item.activo ? 'bg-green-600' : 'bg-gray-300',
                                                item.solicitudes_revisadas === 0 ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'
                                            ]"
                                            :title="item.solicitudes_revisadas === 0 ? 'No ha revisado solicitudes' : (item.activo ? 'Desactivar reconocimiento' : 'Activar reconocimiento')"
                                        >
                                            <span
                                                :class="[
                            ...
                            <tr v-if="!reconocimientos.data || reconocimientos.data.length === 0">
                ...
                <div class="px-6 py-4 border-t border-gray-200 bg-gray-50">
                     <Pagination :links="reconocimientos.links" :total="reconocimientos.total" :from="reconocimientos.from" :to="reconocimientos.to" />
                </div>
            </div>
        </div>
    </AdminLayout>
