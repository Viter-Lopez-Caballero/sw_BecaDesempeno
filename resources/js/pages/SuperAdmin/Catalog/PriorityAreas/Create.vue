...
const submit = () => {
    ...
    alertaCargando('Guardando...', 'Por favor espera mientras se guarda el área prioritaria');
    form.post(route(`${props.routeName}store`), {
        onSuccess: () => {
            cerrarAlerta();
            alertaExito('¡Guardado!', 'El área prioritaria ha sido creada exitosamente');
        },
        onError: () => {
            cerrarAlerta();
            alertaError('Error', 'Hubo un problema al guardar el área prioritaria');
        }
    });
};
</script>
                ...
                <div class="w-full sm:w-auto">
                   <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">{{ title }}</h1>
                    <div class="flex items-center gap-2 mt-2 text-sm">
                        ...
                        <span class="text-gray-900 font-semibold">Agregar Área Prioritaria</span>
                    </div>
                </div>
