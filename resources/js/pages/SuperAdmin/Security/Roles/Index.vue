...
const deleteRole = async (id) => {
    ...
    if (confirmed) {
        router.delete(route(`${props.routeName}destroy`, id), {
            onSuccess: () => {
                alertaExito('¡Eliminado!', 'Rol eliminado correctamente');
            }
        });
    }
};
    ...
    <LayoutAuthenticated>
        ...
        <div class="space-y-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                ...
                <Link v-if="useCan('roles.create')" :href="route(`${routeName}create`)" class="w-full sm:w-auto justify-center px-4 py-2.5 bg-[#1B396A] text-white rounded-lg hover:bg-[#0f2347] transition flex items-center gap-2 font-medium cursor-pointer">
                            ...
                            <tr v-for="(role, index) in roles.data" :key="role.id" class="hover:bg-gray-50 transition">
                                ...
                                <td v-if="useCan('roles.edit') || useCan('roles.delete')" class="px-6 py-4 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <Link v-if="useCan('roles.edit')" :href="route(`${routeName}edit`, role.id)" class="p-2 text-[#1B396A] border border-[#1B396A] rounded-full hover:bg-[#1B396A] hover:text-white transition group cursor-pointer" title="Editar">
                                            <svg xmlns="http://www.w3.org/2000/svg" height="20px" viewBox="0 -960 960 960" width="20px" fill="currentColor">
                                                <path d="M200-200h57l391-391-57-57-391 391v57Zm-80 80v-170l528-527q12-11 26.5-17t30.5-6q16 0 31 6t26 18l55 56q12 11 17.5 26t5.5 30q0 16-5.5 30.5T817-647L290-120H120Zm640-584-56-56 56 56Zm-141 85-28-29 57 57-29-28Z"/>
                                            </svg>
                                        </Link>
                                        <button v-if="useCan('roles.delete')" @click="deleteRole(role.id)" class="p-2 text-red-600 border border-red-600 rounded-full hover:bg-red-600 hover:text-white transition group cursor-pointer" title="Eliminar">
                                            <svg xmlns="http://www.w3.org/2000/svg" height="20px" viewBox="0 -960 960 960" width="20px" fill="currentColor">
