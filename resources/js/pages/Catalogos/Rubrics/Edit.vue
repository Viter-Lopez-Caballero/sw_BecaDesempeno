...
const form = useForm({
    title: props.rubric.data.title,
    is_active: props.rubric.data.is_active,
    questions: props.rubric.data.questions.map(q => ({
        id: q.id,
        text: q.text,
...
const submit = () => {
    ...
    form.put(route('catalogo.rubrics.update', props.rubric.data.id));
};
</script>
