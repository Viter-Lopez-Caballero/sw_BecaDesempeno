...
const email = 'docente1@becas.test';
const password = 'password';

test('register success', async ({ page }) => {
  await page.goto('/register');
  await page.fill('input[id="curp"]', 'JAFD040301HMSMLGA3'); // CURP válido del seeder
  ...
  // Validar que el nombre se autocompleta antes de continuar
  const nameLocator = page.locator('input[id="name"]');
  await expect(nameLocator).toHaveValue(/.+/, { timeout: 10000 });
  // Esperar a que el campo email sea visible, luego rellenarlo solo si está habilitado
  ...
  await page.locator('.vs__dropdown-option', { hasText: 'Instituto Tecnológico de Aguascalientes' }).first().click();
