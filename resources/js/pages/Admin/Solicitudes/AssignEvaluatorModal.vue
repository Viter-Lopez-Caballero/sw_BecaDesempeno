<template>
    <Modal :show="show" @close="close" maxWidth="2xl">
        <div class="p-6">
            ...
            <!-- Solicitud Info -->
            <div class="mb-6 bg-gray-50 p-4 rounded-lg" v-if="solicitud">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <p class="text-xs text-gray-500 uppercase font-semibold">Profesor</p>
                        <p class="text-sm font-medium text-gray-900">{{ solicitud.user?.name }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 uppercase font-semibold">Convocatoria</p>
                        <p class="text-sm font-medium text-gray-900">{{ solicitud.convocatoria?.titulo || 'General' }}</p>
                    </div>
                </div>
            </div>
...
import { useForm, router } from '@inertiajs/vue3'; // Import router for route() usage if needed, but form helper is enough?
...
const props = defineProps({
    show: Boolean,
    solicitud: Object,
    evaluators: Array, // Full list of available evaluators
});
...
const availableEvaluators = computed(() => {
    // Return evaluators NOT in localEvaluators
    // AND optional: NOT already assigned in database (props.solicitud.evaluaciones)
    // Assuming backend handles duplicates, but better UX to hide them.
    const assignedIds = new Set(localEvaluators.value.map(e => e.id));
    
    // Also consider existing ones if we are adding? 
    // The prompt implies this modal is for Initial Assignment primarily.
    // But if we want to be safe:
    if (props.solicitud?.evaluaciones) {
        props.solicitud.evaluaciones.forEach(ev => assignedIds.add(ev.user_id)); // evaluaciones contains pivot-like data? or Evaluacion model with user_id
    }
...
const save = () => {
    form.solicitud_id = props.solicitud.id;
    form.evaluator_ids = localEvaluators.value.map(e => e.id);
    
    form.post(route('admin.solicitudes.assign'), {
        preserveScroll: true,
        onSuccess: () => {
            close();
        }
